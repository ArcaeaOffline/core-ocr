...
`hash` BLOB
        ...
        cls.__insert_property(conn, PROP_KEY_BUILT_TIMESTAMP, str(timestamp))

        conn.executemany(
            """INSERT INTO hashes (`hash_type`, `type`, `label`, `hash`)
                            VALUES (?, ?, ?, ?)""",
            [
                (
                    it.image_hash_type.value,
                    it.category.value,
                    it.image_id,
                    it.hash,
                )
